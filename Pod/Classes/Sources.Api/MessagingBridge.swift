...
public class MessagingBridge : BasePIMBridge, IMessaging, APIBridge {
    ...
    */
    public func sendSMS(number : String , text : String , callback : IMessagingCallback ) {
        ...
        if (logger != nil) {
            logger!.log(ILoggingLogLevel.Debug, category: getAPIGroup()!.toString(), message: "MessagingBridge executing sendSMS...")
        }
    ...
    public override func invoke(request : APIRequest) -> APIResponse? {
        ...
        switch request.getMethodName()! {
            ...
            default:
                // 404 - response null.
                responseCode = 404
                responseMessage = "MessagingBridge does not provide the function '\(request.getMethodName()!)' Please check your client-side API version; should be API version >= v2.2.11."
        }
