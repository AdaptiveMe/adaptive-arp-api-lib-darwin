        ...
        static func fromDictionary(dict : NSDictionary) -> RotationEvent {
            ...
            if let value : AnyObject = dict.objectForKey("destination") {
                if "\(value)" as NSString != "<null>" {
                    resultObject.destination = ICapabilitiesOrientation.toEnum(((value as! NSDictionary)["value"]) as? String)
                }
            }

            if let value : AnyObject = dict.objectForKey("origin") {
                if "\(value)" as NSString != "<null>" {
                    resultObject.origin = ICapabilitiesOrientation.toEnum(((value as! NSDictionary)["value"]) as? String)
                }
            }

            if let value : AnyObject = dict.objectForKey("state") {
                if "\(value)" as NSString != "<null>" {
                    resultObject.state = RotationEventState.toEnum(((value as! NSDictionary)["value"]) as? String)
                }
            }
