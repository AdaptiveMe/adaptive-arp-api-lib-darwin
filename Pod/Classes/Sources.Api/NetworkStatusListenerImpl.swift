...
public class NetworkStatusListenerImpl : BaseListenerImpl, INetworkStatusListener {
    ...
    */
    public func onError(error : INetworkStatusListenerError) { 
        let param0 : String = "Adaptive.INetworkStatusListenerError.toObject(JSON.parse(\"{ \\\"value\\\": \\\"\(error.toString())\\\"}\"))"
    ...
    /**
       ...
       @since v2.0
    */
    public func onResult(networkEvent : NetworkEvent) { 
        let param0 : String = "Adaptive.NetworkEvent.toObject(JSON.parse(\"\(JSONUtil.escapeString(NetworkEvent.Serializer.toJSON(networkEvent)))\"))"
        var listenerId : Int64 = -1
        if (getId() != nil) {
            listenerId = getId()!
        }
    ...
    /**
       ...
       @since v2.0
    */
    public func onWarning(networkEvent : NetworkEvent, warning : INetworkStatusListenerWarning) { 
        let param0 : String = "Adaptive.NetworkEvent.toObject(JSON.parse(\"\(JSONUtil.escapeString(NetworkEvent.Serializer.toJSON(networkEvent)))\"))"
        let param1 : String = "Adaptive.INetworkStatusListenerWarning.toObject(JSON.parse(\"{ \\\"value\\\": \\\"\(warning.toString())\\\"}\"))"
        var listenerId : Int64 = -1
        if (getId() != nil) {
            listenerId = getId()!
        }
