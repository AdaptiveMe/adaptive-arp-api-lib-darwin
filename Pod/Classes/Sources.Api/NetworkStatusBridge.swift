    ...
    public override func invoke(request : APIRequest) -> APIResponse? {
        ...
        switch request.getMethodName()! {
            case "addNetworkStatusListener":
                let listener0 : INetworkStatusListener? =  NetworkStatusListenerImpl(id: request.getAsyncId()!)
                self.addNetworkStatusListener(listener0!);
            case "removeNetworkStatusListener":
                let listener1 : INetworkStatusListener? =  NetworkStatusListenerImpl(id: request.getAsyncId()!)
            ...
            default:
                // 404 - response null.
                responseCode = 404
                responseMessage = "NetworkStatusBridge does not provide the function '\(request.getMethodName()!)' Please check your client-side API version; should be API version >= v2.2.15."
        }
