    ...
    */
    public struct Serializer {
        ...
        static func fromDictionary(dict : NSDictionary) -> ServiceEndpoint {
            let resultObject : ServiceEndpoint = ServiceEndpoint()
            ...
            if let value : AnyObject = dict.objectForKey("validationType") {
                if "\(value)" as NSString != "<null>" {
                    resultObject.validationType = IServiceCertificateValidation.toEnum(((value as! NSDictionary)["value"]) as? String)
                }
            }
